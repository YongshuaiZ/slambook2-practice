...
// 描述子类型 32 bit unsigned int ,will have 8, 8x32=256
...
int main()
{
    ...
    vector<DescType> descriptor1,descriptor2;
    vector<cv::DMatch> matches;

    cv::FAST(image1,keypoints_1,40);
    cv::FAST(image2,keypoints_2,40);

    //描述子
    ComputeORB(image1,keypoints_1,descriptor1);
    ComputeORB(image2,keypoints_2,descriptor2);

    BfMatch(descriptor1,descriptor2,matches);

    // plot the matches
    cv::Mat image_show;
    cv::drawMatches(image1, keypoints_1, image2, keypoints_2, matches, image_show);
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors)
{
    ...
    for(auto &kp:keypoints) {
        if (kp.pt.x <= half_boundary || kp.pt.y <= half_boundary ||
            img.cols - kp.pt.x >= half_boundary || img.rows - kp.pt.y >= half_boundary) {
            bad_points++;
        ...
        for (int dx = - half_patch_size; dx < half_patch_size; dx++) {
            for (int dy = -half_patch_size; dy < half_patch_size; dy++) {
                m10 += dx * img.at<uchar>(kp.pt.y + dy,kp.pt.x+dx);
                m01 += dy * img.at<uchar>(kp.pt.y + dy,kp.pt.x+dx);
            }
        }
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches)
{
    ...
    for(size_t l1 = 0;l1<desc1.size();l1++)
    {
        if(desc1[l1].empty())
            continue;
        cv::DMatch m(l1,0,256);
        for(size_t l2 = 0;l2 < desc2.size();l2++)
        {
            if(desc2[l2].empty())
                continue;
            int distance = 0;
            for (int k = 0; k < 8; k++) {
                // 异或运算，当位上值不同时置1，_mm_popcnt_u32()函数用于统计1的个数
                distance += _mm_popcnt_u32(desc1[l1][k] ^ desc2[l2][k]);
            }
            if(distance <= maxDepth && distance <= m.distance)
            {
                m.trainIdx=l2;
                m.distance= distance;
            }
